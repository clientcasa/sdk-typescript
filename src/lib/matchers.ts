...
import { ClientCasaDefaultError } from "../models/errors/client-casa-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ClientCasaDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | ClientCasaDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ClientCasaDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ClientCasaDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
