 ...
 * List inquiries
 */
export function inquiriesListInquiries(
  ...
  Result<
    operations.ListInquiriesResponse,
    | ClientCasaError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.ListInquiriesRequest$outboundSchema),
        value,
      ),
  ...
  const path = pathToFunc("/api/v1/inquiries")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListInquiriesResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
