...
export function eventDaysList(
  ...
  PageIterator<
    Result<
      operations.ListEventDaysResponse,
      | errors.ApiError
      | ClientCasaError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.ListEventDaysRequest$outboundSchema),
        value,
      ),
  ...
  const path = pathToFunc("/api/v1/event-days")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListEventDaysResponse$inboundSchema, {
      key: "Result",
    }),
