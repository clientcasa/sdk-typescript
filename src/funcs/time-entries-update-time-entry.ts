 ...
 */
export function timeEntriesUpdateTimeEntry(
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.UpdateTimeEntryRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/api/v1/time-entries/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.TimeEntry$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
