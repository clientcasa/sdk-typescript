 ...
 */
export function calendarEventsListCalendarEvents(
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.ListCalendarEventsRequest$outboundSchema),
        value,
      ),
  ...
  const query = encodeFormQuery({
    ...
    "startsBefore": payload?.startsBefore,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListCalendarEventsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
