 ...
 */
export function contractsUpdateContract(
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.UpdateContractRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/api/v1/contracts/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.Contract$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
