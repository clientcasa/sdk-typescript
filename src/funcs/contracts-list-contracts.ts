 ...
 * List contracts
 */
export function contractsListContracts(
  ...
  Result<
    operations.ListContractsResponse,
    | ClientCasaError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.ListContractsRequest$outboundSchema),
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListContractsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
