 ...
 * Get a payout
 */
export function payoutsGetPayout(
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetPayoutRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/api/v1/payouts/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.Payout$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
